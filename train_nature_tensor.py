...
"""
'''
双分支 + 特征向量 + 交叉熵损失函数 + 参数共享
'''

import numpy as np
import math
import time
import model_d_tensor as model
import read_data
...
checkpoint_dir = 'ckpt_tensor'
checkpoint_file = os.path.join(checkpoint_dir, 'model.ckpt')
train_dir='summary'

def initLogging(logFilename='record_tensor.log'):
  """Init for logging
  """
  logging.basicConfig(
                    level    = logging.DEBUG,
...
#        读取数据集
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            try:
                ...
                np.save(os.path.join(checkpoint_dir,'ckpt_tensor'), all_loss)
...
def mc_test(file_name, number):
    ...
    with graph.as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            sess.run(tf.local_variables_initializer())
#            saver.restore(sess, tf.train.latest_checkpoint('ckpt_tensor'))
            saver.restore(sess, 'sia_tensor/model.ckpt-35')
            ...
            try:
                ...
                save_file = open('test_tensor.txt','a')
        ...
        #       绘制ROC曲线
                ...
                save_file.write('\n')
                save_file.close()
                roc_dir = 'plot_curve/nature_tensor/epoch_'+str(number)
                try:
                    os.makedirs(roc_dir)
                except os.error:
                    pass
                np.savez(roc_dir+'/match_tensor_'+file_name, fpr,tpr)
            except KeyboardInterrupt:
                print('INTERRUPTED')
                coord.request_stop()
            finally:
