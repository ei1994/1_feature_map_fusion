...
checkpoint_dir = 'ckpt_map32'
checkpoint_file = os.path.join(checkpoint_dir, 'model.ckpt')
train_dir='summary'

def initLogging(logFilename='record_map32.log'):
  """Init for logging
  """
  logging.basicConfig(
                    level    = logging.DEBUG,
    ...
    with graph.as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            try:
                ...
                np.save(os.path.join(checkpoint_dir,'ckpt_map32'), all_loss)
...
def mc_test(file_name, number):
    ...
    with graph.as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            sess.run(tf.local_variables_initializer())
            saver.restore(sess, tf.train.latest_checkpoint('ckpt_map32'))
#            saver.restore(sess, 'ckpt_tensor/model.ckpt-4000')
            ...
            try:
                ...
                save_file = open('test_map32.txt','a')
        ...
        #       绘制ROC曲线
                ...
                save_file.write('\n')
                save_file.close()
                roc_dir = 'plot_curve/roc/nature_map32/epoch_'+str(number)
                try:
                    os.makedirs(roc_dir)
                except os.error:
                    pass
                np.savez(roc_dir+'/match_map32_'+file_name, fpr,tpr)
            except KeyboardInterrupt:
                print('INTERRUPTED')
                coord.request_stop()
            finally:
