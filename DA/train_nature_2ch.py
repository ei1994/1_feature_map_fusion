...
checkpoint_dir = 'ckpt_2ch'
checkpoint_file = os.path.join(checkpoint_dir, 'model.ckpt')
train_dir='summary'

def initLogging(logFilename='record_2ch.log'):
  """Init for logging
  """
  logging.basicConfig(
                    level    = logging.DEBUG,
    ...
    with graph.as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            try:
                ...
                np.save(os.path.join(checkpoint_dir,'ckpt_2ch'), all_loss)
...
def mc_test(file_name, number):
    ...
    with graph.as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            sess.run(tf.local_variables_initializer())
            saver.restore(sess, tf.train.latest_checkpoint('ckpt_2ch'))
#            saver.restore(sess, 'ckpt_tensor/model.ckpt-4000')
        ...
        #       绘制ROC曲线
                ...
                save_file.write('\n')
                save_file.close()
                roc_dir = 'plot_curve/roc/nature_2ch/epoch_'+str(number)
                try:
                    os.makedirs(roc_dir)
                except os.error:
                    pass
